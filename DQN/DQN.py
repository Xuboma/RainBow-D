...
import numpy as np
from runner import Runner  # Assuming `runner` is a module in your project
...
class DQNAgent:
    ...
    def train(self, state, action, reward, next_state, done, gamma=0.99):
        ...
        with torch.no_grad():
            next_q_values_target = self.q_network(next_state)
...
def train_dqn(agent, env, episodes=100, save_path=None):
    for episode in range(episodes):
        ...
        for i in range(200):
            ...
            if next_state[1] <= 15 and 1 - st_radio > 0.015:
                reward = 0
            elif 15 < next_state[1] < 600 and 1 - st_radio > 0.105:
                reward = 0
        ...
        try:
            df_test = pd.read_excel('DQN_v_data.xlsx')
        ...
        df_test.to_excel('DQN_v_data.xlsx', index=False)

        try:
            df_runtime = pd.read_excel('DQN_runtime_data.xlsx')
        except FileNotFoundError:
        ...
        df_runtime.to_excel('DQN_runtime_data.xlsx', index=False)
...
if __name__ == '__main__':
    ...
    # Train
    train_dqn(agent, env, save_path="trained_q_network.pth")
    # Test
    test_dqn(agent, env)
